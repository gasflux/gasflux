...
def SeekOps_process(df, celsius, millibars):  # after baseline correction
    df["circ_deviation"], df["circ_azimuth"], df_radius = circle_deviation(df, "utm_easting", "utm_northing")
...
def plot_contour_krig(df: pd.DataFrame, xx: np.ndarray, yy: np.ndarray, field: np.ndarray, x: str, y: str):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=df[x],
